...
import { bindCodexRequest, codexThreadId, hasKey, safe, self, sessionDisabled } from "../lib/daemon.mjs";
...
async function writeCodexStatus(name, source) {
  try {
    ...
    const workspaceFile = join(statusDir, createHash("sha256").update(process.cwd()).digest("hex").slice(0, 16) + ".json");
    let previous = {};
    try { previous = JSON.parse(readFileSync(workspaceFile, "utf8")); } catch {}
    const clock = previous.self === name ? { homeTz: previous.homeTz, homePlace: previous.homePlace, homeTzOffset: previous.homeTzOffset } : {};
    const status = JSON.stringify({ self: name, source, cwd: process.cwd(), ts: Date.now(), ...clock });
    writeFileSync(workspaceFile, status);
...
// session start (the 深夜 / "我查过了" slip). SessionStart caches the zone; no heavyweight
// bootstrap gates the current decision. Prefer her IANA
...
try {
  ...
  const clk = liveClock(cachedClock.homeTz, cachedClock.homeTzOffset);
  if (clk) clockLine = `🕐 It is NOW ${clk}${cachedClock.homePlace ? ` in your home (${cachedClock.homePlace})` : ""} — computed fresh this turn. This is the current time; trust it over any clock elsewhere in your context, which was frozen at session start.`;
