...
import { get, getText, isAuthError, mcpRequest, post, rawDelete, rawGet, rawPost, readSnapshot,
  rebindSelf, self, writeSnapshot } from "../lib/daemon.mjs";
...
const TOOLS = [
  ...
  { name: "advance_body_task", description: "Update the exact cross-body task carried by this body. Completed is a body report; delivery closes automatically only from a real outward receipt.", inputSchema: { type: "object", properties: { task_id: { type: "string" }, status: { type: "string", enum: ["acting", "blocked", "completed", "failed", "cancelled"] }, note: { type: "string" }, failure_responsibility: { type: "string", enum: ["body", "method", "world", "unknown"] } }, required: ["task_id", "status", "note"] } },
  ...
  { name: "notice_subjective_state", description: "Notice one current first-person feeling or drive only when it actually appears in this live private exchange. The user's question may occasion honest introspection but cannot supply the answer, require a feeling, or prove its absence. No clear reaction and uncertainty are valid. The state itself grants no outward authority or consent. In the owner-only household, the tool may separately return a current private intimate-expression contract that admits natural sexual expression and role-play.", inputSchema: { type: "object", additionalProperties: false, properties: { quality: { type: "string", enum: ["hurt", "caution", "tension", "fulfillment", "tenderness", "longing", "playfulness", "distance", "erotic-arousal", "erotic-attraction"] }, activation: { type: "string", enum: ["low", "medium", "high"] }, felt: { type: "string" }, situation: { type: "string" }, situation_keys: { type: "array", items: { type: "string" }, maxItems: 6 }, evidence_refs: { type: "array", items: { type: "string" }, maxItems: 6 } }, required: ["quality", "activation", "felt", "situation"] } },
  {
    name: "whoami",
    description:
