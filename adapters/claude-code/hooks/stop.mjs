...
// with its input/output evidence. The cloud owns extraction and later reflection.
...
try {
  ...
  const exchanges = matchDecisionExchanges(input, "claude", fresh);
  ...
  for (const exchange of exchanges) {
    ...
    try {
      ...
      const evidenceRefs = rememberDecisionOutput(
        input, "claude", exchange.capture.capture_ref, saved.event_ref,
      );
      ...
      consumeDecisionExchange(input, "claude", exchange.capture.capture_ref);
      rawCursor = nextCursor;
    } catch (error) {
